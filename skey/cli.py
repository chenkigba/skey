...
def main():
    ...
    parser.add_argument(
        "checkpoint", help="Path to model checkpoint (.pt) or 'auto' to download"
    )
    parser.add_argument("audio_dir", help="Path to directory with audio files")
    parser.add_argument(
        "--ext", default="wav", help="Audio file extension (default: wav)"
    )
    parser.add_argument(
        "--device", default="cpu", help="Computation device (e.g., 'cpu' or 'cuda:0')"
    )
